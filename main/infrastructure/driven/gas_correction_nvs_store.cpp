...
// ── gas history (meter namespace) ─────────────────────────────────────────
// Два ключа: "today" — частый маленький blob (сегодня + текущий час),
// "history" — редкий большой blob (завершённые сутки + вчерашние часы).

bool GasCorrectionNvsStore::load_today_gas(void* blob)
{
    ...
    size_t sz = 0;
    bool ok = false;
    if (nvs_get_blob(n, "today", nullptr, &sz) == ESP_OK && sz == sizeof(GasTodayBlob)) {
        nvs_get_blob(n, "today", blob, &sz);
        ok = true;
    }
    nvs_close(n);
    return ok;
...
void GasCorrectionNvsStore::save_today_gas(const void* blob)
{
    if (!blob || OtaValidityAdapter::is_pending_global()) return;
    nvs_handle_t n;
    if (nvs_open("meter", NVS_READWRITE, &n) != ESP_OK) return;
    nvs_set_blob(n, "today", blob, sizeof(GasTodayBlob));
    nvs_commit(n); nvs_close(n);
}

bool GasCorrectionNvsStore::load_history_gas(void* blob)
{
    if (!blob) return false;
    nvs_handle_t n;
    if (nvs_open("meter", NVS_READONLY, &n) != ESP_OK) return false;
    size_t sz = 0;
    bool ok = false;
    if (nvs_get_blob(n, "history", nullptr, &sz) == ESP_OK && sz == sizeof(GasHistoryBlob)) {
        nvs_get_blob(n, "history", blob, &sz);
        ok = true;
    }
    nvs_close(n);
    return ok;
...
void GasCorrectionNvsStore::save_history_gas(const void* blob)
{
    if (!blob || OtaValidityAdapter::is_pending_global()) return;
    nvs_handle_t n;
    if (nvs_open("meter", NVS_READWRITE, &n) != ESP_OK) return;
    nvs_set_blob(n, "history", blob, sizeof(GasHistoryBlob));
    nvs_commit(n); nvs_close(n);
}
