...
void BurnCycleService::poll()
{
    ...
    if (flame != prev_flame_) {
        if (flame) {
            // Flame ON: record pause duration
            flame_on_ms_ = now_ms;
            if (flame_off_ms_ > 0) {
            ...
            if (flame_on_ms_ > 0) {
                ...
                if (burn < 65535) {
                    ...
                    // idx already advanced by pause, use previous slot
                    int burn_idx = (cycle_idx_ - 1 + RING) % RING;
                    if (cycle_total_ > 0) {
                        burn_dur_[burn_idx] = static_cast<uint16_t>(burn);
                    }
                }
                burner_sec_ += burn;
                cycle_cnt_++;
    ...
    // Accumulate burner seconds for ongoing burn
    if (flame && last_tick_ms_ > 0) {
...
float BurnCycleService::median_burn()  const { return median(const_cast<uint16_t*>(burn_dur_), cycle_total_); }
float BurnCycleService::median_pause() const { return median(const_cast<uint16_t*>(pause_dur_), cycle_total_); }
float BurnCycleService::avg_burn()     const { return average(const_cast<uint16_t*>(burn_dur_), cycle_total_); }
