...
- 🔄 **OTA-обновление с откатом** — A/B-схема: устройство обновляется по воздуху, новая прошивка подтверждает работоспособность за 90 с, при сбое — автоматический откат на предыдущую версию
...
- **Обновление ПО** — OTA-обновление по воздуху: список доступных версий, загрузка, прогресс, ручной откат
...
- **A/B OTA с авто-откатом** — обновление по воздуху через веб-интерфейс; после загрузки прошивка должна подтвердить здоровье (HTTP + цикл) за 90 с, иначе bootloader откатывает на предыдущий слот; контроль целостности NVS во время PENDING_VERIFY
